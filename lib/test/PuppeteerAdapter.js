...
const adapter_1 = __importDefault(require("@pollyjs/adapter"));
...
function getBodyData(response, buffer) {
  ...
  let encoding = type ? content_type_1.default.parse(type).parameters.charset || mime_types_1.default.charset(type) : chardet.detect(buffer);
  ...
  if (encoding && iconv_lite_1.encodingExists(encoding)) {
    const body = iconv_lite_1.decode(buffer, encoding);
    return {
      body,
      isBinary: false
    };
...
class PuppeteerAdapter extends adapter_1.default {
  constructor(polly) {
  ...
  onConnect() {
    this.subscriptions.add(rxjs_1.fromEvent(this.page, 'request').subscribe(request => {
      ...
      if (isPreflight || !this.requestResourceTypes.includes(request.resourceType())) {
        ...
        this.handleRequest({
          ...
          url,
          method,
          body: (_request$postData = request.postData()) !== null && _request$postData !== void 0 ? _request$postData : '',
          requestArguments: {
            request
          }
        });
    ...
    this.subscriptions.add(rxjs_1.fromEvent(this.page, 'response').subscribe(response => {
      const request = response.request();

      if (this.pendingRequests.has(request)) {
        var _this$pendingRequests;

        (_this$pendingRequests = this.pendingRequests.get(request)) === null || _this$pendingRequests === void 0 ? void 0 : _this$pendingRequests.respond(response).catch(error => {
      ...
      if (this.passThroughRequests.has(request)) {
        var _this$passThroughRequ;

        (_this$passThroughRequ = this.passThroughRequests.get(request)) === null || _this$passThroughRequ === void 0 ? void 0 : _this$passThroughRequ.respond(response);
      }
    }));
  }
  /**
  ...
  async passthroughRequest(pollyRequest) {
    ...
    this.passThroughRequests.set(request, {
    ...
    if (response.status() < 300 || response.status() >= 400) {
      let buffer = await response.buffer(); // No idea why, but CDP/Puppeteer report the body as empty for this request.
    ...
    const headers = lodash_1.mapValues(response.headers(), value => value.split('\n').join(', '));
    return {
  ...
  async respondToRequest(pollyRequest, error) {
    ...
    } = pollyRequest; // Do nothing for passthrough requests: Polly calls request.respond() internally.
