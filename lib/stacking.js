...
exports.cleanupStackingLayerChildren = exports.sortStackingLayerChildren = exports.sortChildrenByZIndex = exports.determineStackingLayer = exports.createStackingLayers = exports.establishesStackingContext = void 0;
...
function createStackingLayer(parent, layerName) {
    const layer = parent.ownerDocument.createElementNS(dom_1.svgNamespace, 'g');
...
function determineStackingLayer(styles, parentStyles) {
    ...
    if (css_1.isInFlow(styles) && !css_1.isInline(styles) && !css_1.isPositioned(styles)) {
        return 'inFlowNonInlineNonPositionedDescendants';
    }
    if (!css_1.isPositioned(styles) && styles.float !== 'none') {
        return 'nonPositionedFloats';
    }
    if (css_1.isInFlow(styles) && css_1.isInline(styles) && !css_1.isPositioned(styles)) {
        return 'inFlowInlineLevelNonPositionedDescendants';
    }
    if (zIndex === 0 && (css_1.isPositioned(styles) || establishesStackingContext(styles, parentStyles))) {
